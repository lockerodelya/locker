...
const _ENGINE_JSON_FILE     = 'upsc-math-factors-multiples-level-1.json';
const _ENGINE_CATEGORY_KEY  = 'upsc-math-factors-multiples-level-1';
const _ENGINE_INSTANCE_NAME = 'upscmathfactorsmultiplesl1';
...
// ============================================
    ...
    const _JSON_BASE_PATH = '/vooo-ai/subjects/upsc-math-factors-multiples/json-code/';
...
applyMathFunctions(ev){
    ...
    // ✅ PRE-PASS: flatten (5) → 5 inside all math function arguments only
    ev=ev.replace(/\b(min|max|abs|floor|ceil|round|sqrt|pow|log|log2|log10|gcd|lcm|factorial|sigma|phi|fibonacci|binomial_coefficient|permutations|is_prime|next_prime|next_semiprime|next_abundant|next_deficient|next_perfect|primorial|mod_inverse|sin_degrees|cos_degrees|tan_degrees|round1|round2|round3)\(([^)]*)\)/g,(match,fname,args)=>{
        const cleaned=args.replace(/\((-?\d+(?:\.\d+)?)\)/g,'$1');
...
if(d.hasOwnProperty('A')&&d.hasOwnProperty('STEP')){
    if(!d.hasOwnProperty('B'))d.B=d.A+d.STEP;
    if(!d.hasOwnProperty('C'))d.C=d.A+d.STEP*2;
    if(!d.hasOwnProperty('D'))d.D=d.A+d.STEP*3;
}else if(d.hasOwnProperty('A')){
    if(!d.hasOwnProperty('B'))d.B=d.A+1;
    if(!d.hasOwnProperty('C'))d.C=d.A+2;
    if(!d.hasOwnProperty('D'))d.D=d.A+3;
}
            return d;
        }
...
evaluateBracketExpression(expr,variables){
    let ex=expr;
    ex=ex.replace(/×/g,'*').replace(/÷/g,'/');
    if(/[\+\-\*\^%]/.test(ex)&&!/[?:'"]/.test(ex))
        ex=ex.replace(/(\d)([A-Za-z_][A-Za-z0-9_]*)/g,'$1*$2');
    for(const[k,v]of Object.entries(variables))
        if(typeof v==='string'&&isNaN(v))ex=ex.replace(new RegExp('\\b'+k+'\\b','g'),'"'+v+'"');
    for(const[k,v]of Object.entries(variables))
        if(typeof v!=='string'||!isNaN(v))ex=ex.replace(new RegExp('\\b'+k+'\\b','g'),String(v));
    ex=this.applyMathFunctions(ex);
    try{
        const {argNames,argValues}=this._evalContext();
        const r=new Function(...argNames,'return '+ex)(...argValues);
...
evaluateExpression(expr,variables){
    ...
    ev=ev.replace(/if\s+(.+?)\s+then\s+(.+?)\s+else\s+(.+)/g,(match,cond,thenV,elseV)=>{
        try{
            let c=cond;
            for(const[k,v]of Object.entries(variables))
                c=c.replace(new RegExp('\\b'+k+'\\b','g'),(typeof v==='string'&&isNaN(v))?'"'+v+'"':String(v));
    ...
    for(const[k,v]of Object.entries(variables))
        if(typeof v!=='string'||!isNaN(v))ev=ev.replace(new RegExp('\\b'+k+'\\b','g'),String(v));
    ...
    try{
        if(ev.startsWith('[')&&ev.endsWith(']')){try{return JSON.parse(ev);}catch(err){return[];}}
        ...
        return result;
    }catch(err){console.error('Eval error:',expr,'->',ev,err);return null;}
}
...
if(typeof result==='number'){
    ...
    if(Math.abs(result-rounded)<0.0001)return rounded;
